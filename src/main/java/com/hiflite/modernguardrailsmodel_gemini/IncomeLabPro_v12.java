...
/**
 * Modern Guardrails Retirement Model v12
 * Updates:
 * - Added Inflation_Used_Pct and Market_Return_Used_Pct to CSV.
 * - Added Simulation Average Inflation and Avg Portfolio Withdrawal % to Dashboard.
 */
public class IncomeLabPro_v12 {
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(IncomeLabPro_v12::showGUI);
    }

    private static void showGUI() {
        JFrame frame = new JFrame("Modern Guardrails Retirement Model v12");
        ...
        runBtn.addActionListener(e -> {
            try {
                ...
                frame.dispose();
                new Thread(IncomeLabPro_v12::runSimulation).start();
    ...
    public static void runSimulation() {
        ...
        try (PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter(outputFileName)))) {
            ...
            pw.println("Year,Portfolio_End_Real,Total_Spend_Real,SS_Annuity_Real,Port_Withdrawal_Real,Draw_Pct,Risk_Pct,Inflation_Used_Pct,Market_Return_Used_Pct,Note");
            ...
            for (int year = 0; year <= RETIREMENT_LENGTH; year++) {
                ...
                double drawPctForYear = (startPort > 0 ? (drawReal / startPort) : 0);
                sumDrawPct += drawPctForYear;
                ...
                pw.printf("%d,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%s\n",
                        calYear, Math.max(0, currentPortfolio), spendReal, (ssReal + currentAnnuityReal),
                        drawReal, drawPctForYear * 100, fRisk * 100, simInf * 100, simRet * 100, note);

                if (currentPortfolio <= 0) break;
            }

            double avgInf = sumInf / yearsCounted;
            double avgDraw = sumDrawPct / yearsCounted;
            printDashboard(initialBase, realBaseIncome, currentPortfolio, totalSpent, cuts, raises, maxRisk, calculatePointOfNoReturn(history), avgInf, avgDraw);
    ...
    private static void printDashboard(double start, double end, double port, double spent, int c, int r, double risk, int pnr, double avgInf, double avgDraw) {
        System.out.println("\n========================================================");
        System.out.println("           MODERN GUARDRAILS V12 DASHBOARD              ");
        System.out.println("========================================================");
