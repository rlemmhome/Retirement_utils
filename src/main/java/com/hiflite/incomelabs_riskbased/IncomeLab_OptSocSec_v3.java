 ...
 *
 *  4. TAX ENGINE (v3)
 *     Computed federal + Arizona + IRMAA on the Pro PoS median path. The Tax
 ...
 *     brackets + Arizona 2.5% (which excludes Social Security) + IRMAA (costed,
 *     2-year MAGI lookback). Roth conversions are modeled as a SEPARATE
 ...
 *
 * =============================================================================
 * Compile:  javac IncomeLab_OptSocSec_v3.java
 * Run:      java com.hiflite.incomelabs_riskbased.IncomeLab_OptSocSec_v3
 * Requires Java 11+. No external dependencies.
 */
public class IncomeLab_OptSocSec_v3 extends JFrame {
    ...
    private JPanel buildInputPanel() {
        ...
        spConvCap.setToolTipText("<html><b>Roth conversion cap ($)</b><br>"
            ...
            @Override public String getToolTipText(MouseEvent e) {
                ...
                switch (col) {
                    ...
                    case COL_TAX -> {
                        ...
                        return String.format(
                                "<html><b>Tax (est) -- living-expenses tax (federal + AZ + IRMAA)</b><br>"
                                        ...
                                        + "&nbsp;&nbsp;Federal tax:&nbsp;%s<br>"
                                        + "&nbsp;&nbsp;Arizona 2.5%% (excludes Social Security):&nbsp;%s<br>"
                    ...
                    case COL_CONV_TAX -> {
                        ...
                        double d = showRealDollars ? er.inflFactor : 1.0;
                        return String.format(
                                "<html><b>Conversion tax -- stacked marginal (federal + AZ)</b><br>"
                                        ...
                                        + "minus tax(living), plus Arizona 2.5%%. The conversion does not<br>"
                                        + "get its own standard deduction (already used by spending<br>"
                                        + "income), so it stacks at your top bracket (typically 22%%).<br><br>"
        ...
        JTableHeader hdr = new JTableHeader(tblPro.getColumnModel()) {
            @Override public String getToolTipText(MouseEvent e) {
                ...
                return switch (col) {
                    ...
                    case COL_TAX -> "<html><b>Tax (est) -- computed federal + Arizona + IRMAA</b><br>"
                            ...
                            + "2026 brackets, plus AZ 2.5% and the IRMAA surcharge. When OFF, reverts<br>"
                            + "to the legacy flat escalator. Hover a cell for that year's breakdown.<br>"
                            + "See the Assumptions &amp; Methods tab.</html>";
    ...
    private JScrollPane buildAssumptionsPanel() {
        ...
        String html = ""
                ...
                + "This tool resets withdrawals every run. Both pin PoS; they differ only in when.</p>"
                ...
                + "</ul>"
                + "<p><b>Arizona state tax:</b> flat 2.5% of taxable income.</p>"
                ...
                + "brackets + Arizona 2.5% (which excludes Social Security) + IRMAA. Pre-75 the discretionary "
                ...
                + "living-expenses taxable income: tax(living + conversion) &minus; tax(living), plus Arizona "
                + "2.5%. It does not get its own standard deduction (already consumed by spending income), so "
    ...
    private ProResults simulatePro(SimInputs inp, long seed,
        ...
        for (int y = 0; y < inp.horizon; y++) {
            ...
            if (drawing && inp.computedTax) {
                ...
                double tradDraw = Math.max(combRmd, wdActual);   // RMD floors it
                ...
                double taxSSpre = TaxEngine.taxableSocialSecurity(
                        grossSS, ordinaryBeforeConv, inflFactor);
                ...
                double[] ctax = TaxEngine.conversionTax(
                        tr.taxableIncome, conv, inflFactor);
    ...
    //  TAX ENGINE  (v3)
    // ------------------------------------------------------------------------
    //  Self-contained federal + Arizona + IRMAA calculator for a MFJ couple.
    ...
    static class TaxEngine {
        ...
        // 22% -> 24% MFJ taxable-income boundary (2026); used for conversion cap.
        static final double BRACKET_22_CEILING_TAXABLE_2026 = 211_400.0;
        ...
        static final double[] IRMAA_THRESH_2026    = {218_000, 273_000, 346_000, 750_000};
        static final double[] IRMAA_COUPLE_YR_2026 = {0, 1_188, 3_024, 4_836, 5_508};

        // Social Security provisional-income thresholds (MFJ). Statutory, NOT
         ...
         *  (We treat all non-SS ordinary income as the "other income" base; the
         *  couple has no tax-exempt interest modeled.) MFJ thresholds. */
        static double taxableSocialSecurity(double grossSS, double otherOrdinary,
            ...
            double base   = SS_PROV_BASE_MFJ;
            double second = SS_PROV_SECOND_MFJ;
        ...
        static double fedTax(double taxable, double inflFactor) {
            if (taxable <= 0) return 0;
            double tax = 0;
            for (double[] b : BRACKETS_2026) {
        ...
        static String topBracket(double taxable, double inflFactor) {
            String b = "10%";
            for (double[] br : BRACKETS_2026) {
        ...
        /** Total deduction (base MFJ std ded + age-65 add-on per qualifying
         *  spouse), inflation-indexed. OBBBA senior bonus deliberately omitted. */
        static double totalDeduction(boolean man65, boolean woman65, double inflFactor) {
        ...
        static TaxResult compute(double grossSS, double ordinaryOther,
            ...
            double ded      = totalDeduction(man65, woman65, inflFactor);
            r.taxableIncome = Math.max(0, r.magi - ded);
            r.fedTax        = fedTax(r.taxableIncome, inflFactor);
            // Arizona does NOT tax Social Security -- exclude the taxable SS
            // portion from the AZ base (federal taxable income minus taxable SS).
            ...
            r.topBracket = topBracket(r.taxableIncome, inflFactor);
        ...
        static double[] fillConversion(double magiBeforeConv, double buffer,
            ...
            double irmaaCeil = infl(IRMAA_THRESH_2026[0], inflFactor) - buffer;
            ...
            double ded        = totalDeduction(man65, woman65, inflFactor);
            double bracketCeil = infl(BRACKET_22_CEILING_TAXABLE_2026, inflFactor) + ded;
         ...
         * @return double[]{ totalConvTax, fedConvTax, azConvTax }
         */
        static double[] conversionTax(double livingTaxableIncome, double conversion,
            ...
            double fedWith = fedTax(base + conversion, inflFactor);
            double fedBase = fedTax(base, inflFactor);
            double fedConv = Math.max(0, fedWith - fedBase);
            // Arizona flat 2.5% on the conversion (fully AZ-taxable, not SS).
            double azConv  = conversion * AZ_STATE_RATE;
            return new double[]{ fedConv + azConv, fedConv, azConv };
        }
    }
